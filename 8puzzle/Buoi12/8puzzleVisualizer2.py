...
def solve_and_or_graph_search(start_state, goals):
    ...
    result = or_search(start_state, goals, [])
    if result == "failure": return None, None, None

    # Phân tích kết quả đệ quy thành list các bước duyệt tuyến tính để Visualizer hiểu
    pathState, pathAction, pathStep = [start_state], ["None"], [0]
    step = 1
    curr_result = result
    while curr_result and curr_result != "failure":
        action = curr_result[0]
        plan = curr_result[1]
        if not plan: break

        state_and_plan = plan[0]
        state = state_and_plan[0]
        next_plan = state_and_plan[1]

        pathState.append(state)
        pathAction.append(action)
        pathStep.append(step)

        step += 1
        curr_result = next_plan

    return pathState, pathAction, pathStep


def solve_backtracking_feedforward(start_state, goals, depth_limit=35):
    def bff(state, goals, path, last_action=None, depth_limit=35):
        if is_goal(state, goals):
            return []
        ...
        for new_state, action in neighbors:
            ...
            if new_state not in path:
                plan = bff(new_state, goals, path + [state], action, depth_limit)
                if plan != "failure":
                    return [action, [[new_state, plan]]]
        return "failure"

    result = bff(start_state, goals, [], None, depth_limit)
    if result == "failure": return None, None, None

    # Phân tích kết quả đệ quy thành list các bước duyệt tuyến tính
    pathState, pathAction, pathStep = [start_state], ["None"], [0]
    step = 1
    curr_result = result
    while curr_result and curr_result != "failure":
...
def main(page: ft.Page):
    ...
    algo_dropdown = ft.Dropdown(
        ...
        options=[
            ...
            ft.dropdown.Option("And-Or Graph Search"),
            ft.dropdown.Option("Backtracking Feed Forward"),
        ],
        value="Greedy",
        width=250
    )
    ...
    async def solve_and_animate(e):
        ...
        if selected_algo == "BFS":
            ...
            pathState, pathAction, pathStep = solve_simulated_annealing(start_state, goals, T0=t0_val, Tmin=tmin_val,
                                                                        alpha=alpha_val)
        elif selected_algo == "And-Or Graph Search":
            pathState, pathAction, pathStep = solve_and_or_graph_search(start_state, goals)
        elif selected_algo == "Backtracking Feed Forward":
            pathState, pathAction, pathStep = solve_backtracking_feedforward(start_state, goals, depth_limit=35)
