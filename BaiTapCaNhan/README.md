...
- And-Or Graph Search Phân rã bài toán thành các nhánh hành động (Or) và các nhánh trạng thái kết quả (And). Thuật toán này đặc biệt phù hợp để xử lý tính không tất định của Môi trường quan sát một phần, giúp lập ra một kế hoạch duyệt qua mọi tình huống có thể xảy ra thay vì chỉ một đường đi đơn lẻ.
- Backtracking Feed Forward Kết hợp duyệt quay lui với chiến lược tiến bước an toàn. Thuật toán chủ động loại bỏ các hành động đi ngược lại bước vừa thực hiện (như vừa đi UP lại đi DOWN) và cấm đi lại các trạng thái cũ để tránh lặp vòng. Đồng thời, nó tích hợp giới hạn độ sâu (mặc định là 35) để tránh rơi vào các nhánh vô tận.
